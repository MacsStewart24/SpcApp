...
fun MixRoutineInputForm(
    ...
    LazyColumn(
        ...
        /////////////////////////////////Device Selection/////////////////////////////////////////////////////
        var items2 = itemsRepository.getAllDevicesStream().collectAsState(emptyList())

        var expanded4 by remember { mutableStateOf(false) }
        var selectedOption4 by remember { mutableStateOf<Device?>(null) }
        ...
        ExposedDropdownMenuBox(
            ...
            TextField(
                readOnly = true,
                value = selectedOption4?.deviceName ?: "",
                onValueChange = { },
                ...
                items2.value.forEach { item2 ->
                    DropdownMenuItem(
                        onClick = {
                            ...
                            onMixRoutineValueChange(mixRoutineDetails.copy(
                                deviceId = item2.deviceName,
                                deviceName = item2.deviceName,
                                deviceDescription = item2.deviceDescription,
                            ))
                        }
                    ) {
                        Text(text = item2.deviceName)
                    }
                }
...
//        val durationInSeconds = duration.toSec
            ...
            if (selectedOption2 == ShowTimeInputOption2.SHOW) {
                // Fetching local context
                OutlinedTextField(
                    value = mixRoutineDetails.duration.toString(),
                    onValueChange = { onMixRoutineValueChange(mixRoutineDetails.copy(duration = it.toLong())) },
                    ...
                    DropdownMenuItem(
                        onClick = {
                            ...
                            onMixRoutineValueChange(mixRoutineDetails.copy(
                                ...
                                deviceName2 = item.deviceName,
                                deviceDescription2 = item.deviceDescription,
...
//        val durationInSeconds = duration.toSec
            ...
            if (selectedOption7 == ShowTimeInputOption2.SHOW) {
                OutlinedTextField(
                    value = mixRoutineDetails.duration2.toString(),
                    onValueChange = { onMixRoutineValueChange(mixRoutineDetails.copy(duration2 = it.toLong())) },
                    label = { Text("Duration in seconds") },
                    modifier = Modifier.fillMaxWidth(),
                    enabled = enabled,
                    singleLine = true
